...
{
    class MonthlyDeposit : INotifyPropertyChanged, IDataErrorInfo
    {
        /// <summary>
        ...
        public List<MonthlyDeposit> GetDataList()
        {

            Connection conn = new Connection();
            conn.OpenConection();
            List<MonthlyDeposit> entries = new List<MonthlyDeposit>();
            string query = "SELECT m.*, g.* FROM Member m INNER JOIN MonthlyDepositLedger g  ON m.MemberId = g.MemberId INNER JOIN ( SELECT q.MonthlyId, MAX(MonthlyEntryId) MaxId FROM MonthlyDepositLedger q GROUP BY q.MonthlyId) d ON g.MonthlyId=d.MonthlyId AND g.MonthlyEntryId=d.MaxId ORDER BY d.maxid DESC";
            SqlDataReader reader = conn.DataReader(query);
            while (reader.Read())
            {
                entries.Add(new MonthlyDeposit()
                {
                    MemberId = (int)reader["MemberId"],
                    Name = (string)reader["MemberName"],
                    AccountNo = (int)reader["MonthlyId"],
                    Balance = (double)reader["MonthlyBalance"],
                });
            }
            conn.CloseConnection();
            return entries;
        }
        public List<MonthlyDeposit> GetDataLedger(int id, int front)
        {
            ...
            if (id == 0)
            {
                query1 = "SELECT TOP 1 * FROM MonthlyDepositLedger ORDER BY MonthlyId ASC";
            }
            else if (front == 1)
            {
                query1 = "SELECT TOP 1 * FROM MonthlyDepositLedger WHERE MonthlyId > " + id + " ORDER BY MonthlyId ASC";
            }
            else if (front == 0)
            {
                query1 = "SELECT TOP 1 * FROM MonthlyDepositLedger WHERE MonthlyId < " + id + " ORDER BY MonthlyId DESC";
            }
            else
            {
                query1 = "SELECT TOP 1 * FROM MonthlyDepositLedger WHERE MonthlyId = " + id;
            }
            conn1.OpenConection();
            ...
            conn.OpenConection();
            List<MonthlyDeposit> entries = new List<MonthlyDeposit>();
            string query = "SELECT * From MonthlyDepositLedger where MonthlyId = " + id;
            SqlDataReader reader = conn.DataReader(query);
            while (reader.Read())
            {
                entries.Add(new MonthlyDeposit()
                {
                    ID = (int)reader["MonthlyEntryId"],
                    AccountNo = (int)reader["MonthlyId"],
                    MemberId = (int)reader["MemberId"],
                    Date = (DateTime)reader["MonthlyDate"],
                    Details = (string)reader["MonthlyDetails"],
                    Deposit = (double)reader["MonthlyDeposit"],
                    Withdraw = (double)reader["MonthlyWithdraw"],
                    Balance = (double)reader["MonthlyBalance"],
            ...
            query = "SELECT TOP 1 * FROM MonthlyDepositLedger ORDER BY MonthlyEntryId DESC";
            conn.OpenConection();
            reader = conn.DataReader(query);
            while (reader.Read())
            {
                m_id = (int)reader["MonthlyEntryId"] + 1;
            }
        ...
        public void GetDataDetails(int id)
        {
            ...
            while (reader.Read())
            {
                ...
                SNominee = (string)reader["MDNomineeName"];
        ...
        public void GetDataDetailsUnknown(string memberUnknown)
        {
            Connection conn = new Connection();
            conn.OpenConection();
            string query = "SELECT m.*, g.* From Member m LEFT JOIN MonthlyDepositDetails g ON m.MemberId=g.MemberId";
            SqlDataReader reader = conn.DataReader(query);
            ...
            if (checkExistence == 0)
            {
                MessageBox.Show("Member may not have a monthly deposit account.Check Input again.\n", "Warning", MessageBoxButton.OK, MessageBoxImage.Exclamation);
            }
